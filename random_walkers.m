...
% that you tell the program to use and the results (figures, csv) it outputs
...
% 6. organize photos and csv's into a folder
...
color_cell = 0; % 1 to turn on to bin cell density and graph it
...
tmax = 100; % set simulation time duration (goal is 10 mins) %$
...
n = 20000; % can go up to 10e5 cells without much problem % 10000
...
vmax = 40; % scales the velocity distribution % 20 %$
pstuck = 0.1; %$
...
% title(strcat(num2str(t),' seconds'))
xlabel('Distance (um)')
ylabel('Cell Density (orange) & Food Density (blue)')

hold on
h6 = plot(Y_h3,A1_t); % A1_filt if you want to see the smoother version
...
saveas(gcf,strcat(folder,'peak_',num2str(avg_peak_vel),'.jpg'))
...
labels = {'density_3d' 'n' 'x' 'y' 'tmax' 'tstep' 'vmax' 'pstuck' 'athresh' 'max_run' 'min_run' 'alpha' 'cons' 'slope' 'avg_peak_vel' 'R2' 'avg_peak_size' 'winsize'};
values = [density_3d n x y tmax tstep vmax pstuck athresh max_run min_run alpha cons slope avg_peak_vel R2 avg_peak_size winsize];

file = strcat(folder,rt_graph_setting,'_',num2str(avg_peak_vel),'.csv');
fid = fopen(file, 'w') ;
 fprintf(fid, '%s,', labels{1,1:end-1}) ;
 fprintf(fid, '%s\n', labels{1,end}) ;
 fclose(fid) ;
 dlmwrite(file, values, '-append') ;
